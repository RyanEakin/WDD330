import { setLocalStorage, ids, LoadHeaderFooter, getLocalStorage } from "./utils.mjs";
...
if (localStorage.length === 0) {
    const MovieRequest = await marathon.getMediaData("movie", "", "");
    const MoviePlayRequest = await marathon.getMediaData("movie", "now_playing", "");
    const MoviePopRequest = await marathon.getMediaData("movie", "popular", "");
}
...
mediaL.init();
mediaL.renderList(ids("movie"), Movies, tvList);
mediaL.renderList(ids("popular"), Pop, tvList);

const mediaL_play = new Recommendations(); // made it simplier, because... renderList is the ONLY function that needs datasource
mediaL_play.init();
mediaL_play.renderList(ids("now_playing"), Play, tvList);
